...
record Ok<T, E>(T value) implements Result<T, E> {
    ...
    public <U> U match(Function<Result<T, E>, U> defaultArm, List<MatchArm<T, E, U>> matchArms) {
        return MatchArm.okArms(matchArms).filter(e -> e.matches(this.value)).map(e -> e.map(this.value)).findFirst().orElseGet(() -> defaultArm.apply(this));
    }

    @Override
    public <U> U match(Function<T, U> defaultOk, Function<E, U> defaultErr, List<MatchArm<T, E, U>> matchArms) {
        return MatchArm.okArms(matchArms).filter(e -> e.matches(this.value)).map(e -> e.map(this.value)).findFirst().orElseGet(() -> defaultOk.apply(this.value));
    }
